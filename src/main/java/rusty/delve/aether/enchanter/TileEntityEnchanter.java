...
public class TileEntityEnchanter extends TileEntity implements IInventory, ITickable {
	...
	public String getName() {
		return this.hasCustomName() ? this.customName : "container.enchanter";
	}
	...
	public void update() {
		...
		if(!this.world.isRemote) {
			...
			if(flag != this.isBurning()) {
				flag1 = true;
				BlockEnchanter.setState(this.isBurning(), this.world, this.pos);
			}
		}
	...
	//Importante
	public int getCookTime(ItemStack fuel, ItemStack input) {
		return 200;
	}
	...
	private boolean canSmelt() 
	{
		...
		else 
		{
			...
			ItemStack result = EnchanterRecipes.getInstance().getEnchantingResult((ItemStack)this.inventory.get(1), 
																				  ...
																				  (ItemStack)this.inventory.get(4));
	...
	public void smeltItem() {
		if(this.canSmelt()) {
			...
			ItemStack result = EnchanterRecipes.getInstance().getEnchantingResult(input1, input2, input3, input4);
			ItemStack output = (ItemStack)this.inventory.get(5);
	...
	public static int getItemBurnTime(ItemStack fuel) {
		if(fuel.isEmpty()) return 0;
			...
			if(item == DelveItems.AMBROSINE) return 100;
			if(item == DelveItems.AMBROSINE_DUST) return 200;
			if(item == ItemsAether.ambrosium_shard) return 400;
			if(item == DelveItems.AMBROSINE_CHUNK) return 1000;
			if(item == DelveItems.AMBROSINE_CRYSTAL) return 2500;
			if(item == DelveItems.AETHERFLUX) return 2500;
			if(item == DelveItems.AETHERFLUX_CRYSTAL) return 2500;
			return 0;
		}
	}
